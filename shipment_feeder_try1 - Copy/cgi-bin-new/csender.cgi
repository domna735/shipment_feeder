...
# debug print
print "DEBUG: start cgi<br>\n";
print "DEBUG: cookie SESSIONID=$session_token<br>\n";
print "DEBUG: origin=" . $cgi->param('origin') . "<br>\n";
print "DEBUG: sub_grp_id=" . $cgi->param('sub_grp_id') . "<br>\n";
print "DEBUG: paper_ty=" . $cgi->param('paper_ty') . "<br>\n";
print "DEBUG: rload=" . $cgi->param('rload') . "<br>\n";

# 參數處理
my $origin = $cgi->param('origin');
my $sub_grp_id = $cgi->param('sub_grp_id');
my $paper_ty = $cgi->param('paper_ty');
my $rload = $cgi->param('rload');

# session 驗證
unless (is_valid_session($session_token)) {
    print "Session invalid, redirecting...<br>\n";
...
sub is_valid_session {
    ...
    my $session_file = "/appl/service/webapps/shipment_feeder/sessions/$token";
    print "DEBUG: token=$token<br>\n";
    print "DEBUG: session_file=$session_file<br>\n";
    if (-e $session_file) {
        print "DEBUG: session file exists<br>\n";
        open my $fh, '<', $session_file or print "DEBUG: cannot open session file<br>\n";
        my $content = <$fh>;
        print "DEBUG: session file content=$content<br>\n";
        close $fh;
    } else {
        print "DEBUG: session file not found<br>\n";
    }
